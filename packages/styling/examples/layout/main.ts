...
const divider = StyleBuilder.create().setString('•').padding(0, 1).foreground(subtle).render('');
...
const tab = StyleBuilder.create().border(tabBorder).borderForeground(highlight).padding(0, 1);
...
const tabGap = StyleBuilder.create().border(tabBorder).borderForeground(highlight).padding(0, 1);
...
const infoStyle = StyleBuilder.create().border(Border.normal()).borderForeground(subtle);

// Dialog
const dialogBoxStyle = StyleBuilder.create().border(Border.rounded()).padding(1, 0);
...
const checkMark = StyleBuilder.create().foreground(special).paddingRight(1).render('✓');

const listDone = (s: string) =>
  checkMark +
  StyleBuilder.create().strikethrough(true).foreground(lightDark('#969B86', '#696969')).render(s);
...
// Status Bar
const statusNugget = StyleBuilder.create().foreground('#FFFDF5').padding(0, 1);
...
const fishCakeStyle = StyleBuilder.create().inherit(statusNugget.build()).background('#6124DF');
...
// Dialog
{
  ...
  const dialogContent = Layout.joinVertical(
    0.5,
...
// Lists
const lists = Layout.joinHorizontal(
  0.0, // top alignment
  list.render(
    Layout.joinVertical(
      0.0, // left alignment
