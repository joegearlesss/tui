 ...
 */
export const LayerPositionSchema = z
  .object({
    x: z
      .number()
...
export const CanvasConfigSchema = z
  .object({
    layers: z.array(z.any()).describe('Array of Layer instances to be composited on the canvas'),
  })
 ...
 */
export const LayerDimensionsSchema = z
  .object({
    width: z
      .number()
      .int('Width must be an integer')
